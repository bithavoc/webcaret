// D import file generated from 'heaploop/looping.d'
...
template OperationContext(T : Looper)
{
		...
		private 
		{
			...
			public 
			{
				this(T target)
				{
					...
					debug (1)
					{
						std.stdio.writeln("new OperationContext");
					}
				...
				~this()
				{
					debug (1)
					{
						std.stdio.writeln("OperationContext destroyed");
					}
				...
				void resume()
				{
					debug (1)
					{
						std.stdio.writeln("Trying to resume while the fiber is in state ", _fiber.state);
					}
				...
				@property pure nothrow bool hasError()
				{
					return error.isError;
				}


				@property nothrow Fiber fiber()
				{
					return _fiber;
				}


				nothrow @property target()
				{
					return _target;
				}
...
@property pure nothrow bool isError(int status)
{
	return status < 0;
}


@property pure nothrow bool isError(duv_error error)
{
...
class LoopException : Exception
{
	private 
	{
		...
		public 
		{
			...
			@property pure nothrow string name()
			{
				return _name;
			}
		...
		protected 
		{
			...
			public 
			{
				this(Loop loop, uv_handle_type type)
				{
					...
					debug (1)
					{
						std.stdio.writeln("(Handle just initialized OPEN handle ", _handle, ")");
					}
				...
				~this()
				{
					debug (1)
					{
						std.stdio.writeln("Destroying handle");
					}
				...
				@property 
				{
					...
					pure nothrow uv_handle_t* handle()
					{
				...
				void close(bool async = true)
				{
					if (!isOpen)
					{
						debug (1)
						{
							std.stdio.writeln("(tried to close but handle ", _handle, " was reported to be closed or closing already)");
						}
					...
					debug (1)
					{
						std.stdio.writeln("(about to CLOSE handle ", _handle, ") of type ", this);
					}

					closeCleanup(true);
					debug (1)
					{
						std.stdio.writeln("closing handle async");
					}

					duv_handle_close_async(this.handle);
					debug (1)
					{
						std.stdio.writeln("closed handle async");
					}
