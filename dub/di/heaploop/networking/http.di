// D import file generated from 'heaploop/networking/http.d'
...
abstract class HttpConnectionBase : Looper
{
	private 
	{
		...
		void _onMessageBegin(HttpParser p)
		{
			debug (1)
			{
				std.stdio.writeln("HTTP message began");
			}
		...
		void onHeadersComplete(HttpParser p)
		{
			...
			debug (1)
			{
				std.stdio.writeln("protocol version set, ", p.protocolVersion.toString);
			}
		...
		void onBody(HttpParser parser, HttpBodyChunk chunk)
		{
			debug (1)
			{
				writeln("HTTP Response Message: read some BODY data: ", chunk.buffer);
			}
		...
		void _stopProcessing()
		{
			...
			debug (1)
			{
				std.stdio.writeln("_Stopping connection, closing stream");
			}
			...
			debug (1)
			{
				std.stdio.writeln("_Stopping connection, OK");
			}
		...
		~this()
		{
			debug (1)
			{
				std.stdio.writeln("Collecting HttpConnection");
			}
		...
		package 
		{
			...
			protected 
			{
				final @property 
				{
					pure nothrow HttpIncomingMessage currentMessage()
					{
						return _currentMessage;
					}

					pure nothrow HttpParser parser()
					{
						return _parser;
					}
				...
				void startProcessing()
				{
					try
					{
						debug (1)
						{
							std.stdio.writeln("Reading to Process HTTP Requests");
						}
					...
					catch(LoopException lex)
					{
						if (lex.name == "EOF")
						{
							debug (1)
							{
								std.stdio.writeln("Connection closed");
							}
					...
					debug (1)
					{
						std.stdio.writeln("Stopped processing requests");
					}
				...
				public 
				{
					...
					void stop()
					{
						debug (1)
						{
							std.stdio.writeln("Stopping connection");
						}

						_stopProcessing;
					}

					@property pure nothrow Loop loop()
					{
						return _stream.loop;
					}
...
abstract template HttpConnection(TIncomingMessage : HttpIncomingMessage)
{
	class HttpConnection : HttpConnectionBase
	{
		protected 
		{
			...
			public this(TcpStream stream, HttpParserType parserType)
			{
	...
	private 
	{
		...
		package 
		{
			...
			public 
			{
				@property 
				{
					pure nothrow Loop loop()
					{
						return _loop;
					}

					pure nothrow HttpHeader[] headers()
					{
						return _headers;
					}
...
abstract class HttpIncomingMessage : HttpMessage
{
	private 
	{
		...
		package 
		{
			...
			@property void transmissionMode(HttpBodyTransmissionMode mode)
			{
			...
			public 
			{
						...
						if (this.shouldRead)
						{
							...
							do
							{
								if (cx.hasBufferedChunk)
								{
									debug (1)
									{
										writeln("HTTP Response Message: delivering buffered data");
									}
				...
				@property 
				{
					...
					pure nothrow bool shouldRead()
					{
						return this.transmissionMode.shouldRead;
					}

					pure nothrow HttpBodyTransmissionMode transmissionMode()
					{
...
class HttpResponse
{
	private 
	{
		...
		{
			_connection.write(cast(ubyte[])(data ~ "\x0d\x0a"));
		}
		...
		package 
		{
			...
			public 
			{
				...
				void write(ubyte[] data)
				{
					...
					{
						_ensureHeadersSent();
						_connection.write(cast(ubyte[])format("%x\x0d\x0a", data.length));
						_connection.write(data ~ cast(ubyte[])"\x0d\x0a");
					}
					else
					{
				...
				void end()
				{
					...
					else
					{
						...
						debug (1)
						{
							std.stdio.writeln("Closing");
						}

						debug (1)
						{
							std.stdio.writeln("...Closed");
						}

					}
					debug (1)
					{
						std.stdio.writeln("...Ended");
					}
	...
	public 
	{
		...
		ushort inferPortForUriSchema(string schema)
		{
			switch (schema)
			{
				...
				default:
				{
					assert(0, "Unable to infer port number for schema %s".format(schema));
				}
			}
		}
			...
			private 
			{
				...
				public 
				{
					...
					void send(TcpStream stream)
					{
						...
						{
							stream.write(cast(ubyte[])(s ~ "\x0d\x0a"));
						}
						;
		...
		{
			public this(HttpClientConnection connection)
			{
				super(connection);
			}
		...
		{
			public abstract void writeTo(void delegate(ubyte[] data) writer);
		...
		{
			public this(string[string] fields)
			{
		...
		class HttpClient
		{
			private 
			{
				...
				public 
				{
					...
					@property pure nothrow Uri rootUri()
					{
						return _rootUri;
					}
